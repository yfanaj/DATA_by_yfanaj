module mod_clm_initimeconst
  ...
  subroutine iniTimeConst
    ...
    real(rk8) , pointer :: gwp(:)  ! yfanaj 29-Oct-2024
    real(rk8) , pointer :: gwp_tmp(:)  ! yfanaj 29-Oct-2024
    real(rk8) , pointer :: mad(:)  ! yfanaj 26-Dec-2024
    real(rk8) , pointer :: mad_tmp(:)  ! yfanaj 26-Dec-2024
    ...
    gwp             => clm3%g%l%c%cws%gwp ! yfanaj 29-Oct-2024
    mad             => clm3%g%l%c%cws%mad ! yfanaj 29-Oct-2024
    ...
    allocate(gwp_tmp(begg:endg))
    call clm_readvar(ncid,'GWP',gwp_tmp,gcomm_gridcell)
    do c = begc , endc
      g = cgridcell(c)
      gwp(c)=gwp_tmp(g)
    end do
    ...
    allocate(mad_tmp(begg:endg))
    call clm_readvar(ncid,'MAD',mad_tmp,gcomm_gridcell)
    do c = begc , endc
      g = cgridcell(c)
      mad(c)=mad_tmp(g)
    end do
