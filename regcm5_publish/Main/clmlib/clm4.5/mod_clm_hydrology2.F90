module mod_clm_hydrology2
  ...
  subroutine Hydrology2(lbc, ubc, &
    ...
    real(rk8), pointer :: gwp(:)   ! yfanaj on 18-Dec-2024
    ...
    gwp               => clm3%g%l%c%cws%gwp ! yfanaj 2024-12-18
    ...
    do fc = 1 , num_nolakec
      ...
      if ((ityplun(l)==istsoil .or. ityplun(l)==istcrop) &
           .and. cactive(c)) then
        ! yfanaj change to partial groundwater extraction on 2024-12-18
        qflx_runoff(c) = qflx_runoff(c) - qflx_irrig(c)*(1.0_rk8-gwp(c))
      end if
      if (ityplun(l)==isturb) then
