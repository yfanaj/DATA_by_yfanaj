module mod_clm_balancecheck
  ...
  subroutine BalanceCheck(lbp,ubp,lbc,ubc,lbl,ubl,lbg,ubg)
    ...
    real(rk8) , pointer , dimension(:) :: wa ! yfanaj 17-Jan-2026
    real(rk8)  :: errh2ogw ! yfanaj add this for new criteria
    real(rk8) , pointer , dimension(:) :: gwp ! yfanaj 29-Oct-2024
    ...
    wa                  => clm3%g%l%c%cws%wa ! yfanaj 17-Jan-2026
    ...
    found = .false.
    do c = lbc , ubc
      errh2ogw = errh2o(c)+qflx_irrig(c)*gwp(c)*dtsrf
      ! yfanaj change this to half half
      !  if ( abs(errh2o(c)) > 1e-7_rk8 ) then
      if ( abs(errh2o(c)) > 1e-7_rk8 .and.  &
           abs(errh2ogw) > 1e-7_rk8) then
        found = .true.
        indexc = c
      end if
    end do
    ...
    if ( found ) then
      ...
      if ( (ctype(indexc) == icol_roof .or.        &
        ...
        write(stderr,*)'endwb        = ',endwb(indexc)
        write(stderr,*)'begwb        = ',begwb(indexc)
        write(stderr,*)'wa           = ',wa(indexc) ! yfanaj add 17-Jan-2026
        write(stderr,*)'qflx_evap_tot= ',qflx_evap_tot(indexc)
        write(stderr,*)'qflx_irrig   = ',qflx_irrig(indexc)
        write(stderr,*)'qflx_surf    = ',qflx_surf(indexc)
