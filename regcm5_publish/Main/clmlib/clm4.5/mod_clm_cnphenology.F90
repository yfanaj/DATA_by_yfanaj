module mod_clm_cnphenology
  ...
  subroutine CropPhenology(num_pcropp, filter_pcropp)
    ...
    do fp = 1, num_pcropp
      ...
      ! yfanaj warning: planted_crop_day is shared so only one grid is initialied
      ! CLM5 use mcsec but when timestep is special, it may not have mcsec=0
      ! yfanaj replace planted_crop_day to mcsec on 2024-Dec-30
      ...
      if ( jday == jdayyrstart(h) .and. mcsec == 0) then
          ...
          ! yfanaj allow continuous grow for winterwheat
        ...
        if (ivt(p) == nwcereal .or. ivt(p) == nwcerealirrig) then
          ...
          if (t10(p) /= spval.and. a10tmin(p) /= spval   .and. &
            ...
            if (ivt(p)==ncorn .or. ivt(p)==ncornirrig) then
              ...
              ! yfanaj comments: Wendy think this line can be commented.
      ...
      if (croplive(p)) then
        ...
        if (leafout(p) >= huileaf(p) .and. &
          ...
          ! yfanaj add crop changes between sc and ww on Jan-09-2024
          croplive(p) = .false. ! no re-entry in greater if-block
          if (harvdate(p) >= NOT_Harvested) then
            harvdate(p) = jday
            if (harvdate(p) <= 200._rk8 .and. ivt(p) == nwcerealirrig) then
              ivt(p)= ncornirrig
              cropplant(p) = .false.
              idop(p)      = NOT_Planted
            else if (harvdate(p) >= 200._rk8 .and. ivt(p) == ncornirrig) then
              ivt(p)= nwcerealirrig
              cropplant(p) = .false.
              idop(p)      = NOT_Planted
            else if (harvdate(p) <= 200._rk8 .and. ivt(p) == nwcereal) then
              ivt(p)= ncorn
              cropplant(p) = .false.
              idop(p)      = NOT_Planted
            else if (harvdate(p) >= 200._rk8 .and. ivt(p) == ncorn) then
              ivt(p)= nwcereal
              cropplant(p) = .false.
              idop(p)      = NOT_Planted
            end if  
