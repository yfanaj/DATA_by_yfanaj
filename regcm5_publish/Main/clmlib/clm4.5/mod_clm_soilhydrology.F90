module mod_clm_soilhydrology
  ...
  subroutine Drainage(lbc, ubc, num_hydrologyc, filter_hydrologyc, &
    ...
    real(rk8), pointer :: watsat(:,:)
    real(rk8), pointer :: hkdepth(:)    ! decay factor (m)
    real(rk8), pointer :: gwp(:)        ! yfanaj on 29-Oct-2024
    real(rk8), pointer :: zwt(:)        ! water table depth (m)
...
!-----------------------------------------------------------------------
    ...
    rous = 0.2_rk8 ! yfanaj this may need to change later
    ...
    do fc = 1 , num_hydrologyc
      ...
      rous = max(rous,0.02_rk8) ! yfanaj this can be changed to 1.0 rk8
      
      ! water table is below the soil column
      if ( jwt(c) == nlevsoi ) then
        ! yfanaj add this partial gw extraction on 27-Oct-2024

        wa(c)  = wa(c) + (qcharge(c)-qflx_irrig(c)*gwp(c))  * dtsrf
        zwt(c) = zwt(c) - ((qcharge(c)-qflx_irrig(c)*gwp(c)) * dtsrf)/1000.0_rk8/rous
      else
        ! water table within soil layers 1-9
        ! try to raise water table to account for qcharge
        ! yfanaj add this partial gw extraction on 29-Oct-2024
        qcharge_tot = (qcharge(c)-qflx_irrig(c)*gwp(c)) * dtsrf
